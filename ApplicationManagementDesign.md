...
| **Nginx Config** | `nginx.conf` | Optional | Nginx server configuration for the application. |
...
- `labels`: A dictionary of localized names.
- `path`: The relative subdirectory name used under `${MILLEGRILLES_ROOT}/var/nginx/html/applications/`.

#### `nginx.conf`
If present, this file is renamed to `${app_name}.conf` and placed in `${MILLEGRILLES_ROOT}/etc/nginx/applications/`.
...
- **URL Format**: `https://<catalogue_host>/catalogue/{env}.json`
- **Entry Structure**:
  ```json
  {
    "app_name": {
      "url": "https://<download_host>/archives/app_name.tar.gz",
      "version": "2026.1.2",
      "sha256": "abcdef123456...",
      "labels": {
        "en": "English Name",
...
- **`install [--name <name> [--version <version>] [--env <dev|test|stable>] [--catalogue_url <url>]] | [--url <url> [--hash <hash>]] [--root <path>]`**
  - Resolves application details via the remote catalogue if `--name` is used (defaults to `https://localhost/catalogue/stable.json`).
  - Downloads and verifies the package.
  - Configures Nginx, Docker Compose, and Application files.
...
```nginx
include /etc/nginx/applications/*.conf;
```

### Service Lifecycle
All installation and uninstallation actions trigger a restart of the Nginx service:
