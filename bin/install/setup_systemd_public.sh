...
# Bare hook for nginx (no effect unless modified manually)
generate_service "$TEMPLATE_DIR/nginx_public.service.template" "$DEST_DIR/${INSTANCE_NAME}-nginx.service"

# Generate node manager: ${INSTANCE_NAME}.service
generate_service "$TEMPLATE_DIR/manager_nodocker.service.template" "$DEST_DIR/${INSTANCE_NAME}-manager.service"
