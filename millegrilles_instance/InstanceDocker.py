...
from millegrilles_messages.docker.ParseConfiguration import ConfigurationService, Archive
...
def installer_archive(archive: Archive):
    module, sub_path = archive.location.split(':')

    if module == 'nginx':
        module_path = '/var/opt/millegrilles/nginx'
    else:
        raise Exception('Module %s is not supported' % module)

    app_url = urlparse(archive.src)
    filename = app_url.path.split('/')[-1]
    ...
    file_download_path = pathlib.Path(download_path, filename)
